...
last_modified_at: 2025-10-21 00:10:00 +0900
...
1. **임베딩(Embedding):** 단어를 '의미가 담긴 숫자'로 바꾸는 기술

2. **잔차 연결(Residual Connection):** 똑똑한 AI(깊은 신경망)를 학습시킬 수 있게 만든 혁신
...
> **현대 모델의 토큰화 방식**
>
> 본문에서는 쉬운 이해를 위해 간단하게 어절 단위로 설명했지만, BERT나 GPT 같은 현대 모델들은 '**서브워드(Subword)**' 토큰화 방식(예: BPE, SentencePiece)를 사용합니다.
>
...
* $Vector('왕') - Vector('남자') + Vector('여자') \approx Vector('여왕')$
* $Vector('서울') - Vector('한국') + Vector('일본') \approx Vector('도쿄')$
...
기울기 소실 문제를 어떻게 해결했을까요? 2015년에 '**ResNet(Residual Network)**'이라는 논문에서 아주 단순하면서도 강력한 아이디어가 제시되는데, 이것이 바로 '**잔차 연결(Residual Connection)**'입니다.
...
1. **임베딩(Embedding):** AI가 단어의 의미를 이해할 수 있도록 좌표(벡터)로 만드는 기술

2. **잔차 연결(Residual Connection):** 기울기 소실을 해결하고 신경망을 깊이 학습시킬 수 있게 한 방법

다음 포스트인 [2부 - 현대 AI의 심장, 트랜스포머](/posts/ai2-transformers-the-heart-of-modern-ai/)에서는 이 두 가지 핵심 부품을 토대로, 현존하는 거의 모든 LLM의 심장부인 **'트랜스포머(Transformer)'** 구조에 관하여 본격적으로 살펴보겠습니다.
