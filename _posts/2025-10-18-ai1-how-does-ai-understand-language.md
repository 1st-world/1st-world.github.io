...
>
> 워드 임베딩은 이 개념을 '의미'로 확장하는 것입니다. '왕'과 '여왕'은 의미적으로 가깝고, '왕'과 '사과'는 멀도록 벡터 값을 부여합니다.
{: .prompt-tip }
...
이처럼 텍스트(토큰)를 의미가 담긴 벡터로 변환(임베딩)하는 전체 과정을 **벡터화(Vectorize)**라고 합니다.
...
>
> 하지만 "bank"의 의미는 상황에 따라 '은행(go to the bank)'이 될 수도, '강둑(on the river bank)'이 될 수도 있겠죠. 즉, 문맥이 중요합니다.
>
...
>
> 이 구조는 신경망 층($F(x)$)이 학습하는 목표를 바꿔줍니다.
>
