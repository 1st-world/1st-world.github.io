---
title: "클라우드 컴퓨팅과 가상화의 개념"
author: 1st-world
date: 2025-10-12 23:50:00 +0900
last_modified_at: 2025-12-16 22:45:00 +0900
categories: [Cloud]
tags: [iaas, paas, saas, virtualization, hypervisor, container]
pin: false
---
...
**클라우드 컴퓨팅**은 인터넷을 통해 컴퓨팅 자원(서버, 스토리지, 데이터베이스 등)을 빌려 쓰는 서비스입니다. 온-프레미스(On-Premise) 방식과 비교했을 때 여러 장점들이 있어 각광받고 있습니다.
...
| **Hybrid Cloud** | Public, Private 클라우드를 혼용하는 방식입니다. | 유연성 극대화<br>민감 데이터는 Private,<br>일반 데이터는 Public 활용 |
{: style="width: 100%;"}
...
하이퍼바이저의 종류는 크게 2가지로 나눌 수 있습니다.

| 구분 | 설명 | 용도 | 예시 |
| --- | --- | --- |
| **Type 1** | 하드웨어 위에서 직접 실행 | 서버 가상화, 클라우드 | VMware ESXi, Microsoft Hyper-V, KVM |
| **Type 2** | 기존 운영체제 위에서 실행 | 개인용, 개발/테스트 환경 | VMware Workstation, Oracle VirtualBox |
{: style="width: 100%;"}
...
- **격리 보장:** 특정 VM에서 발생한 장애가 다른 VM에 영향을 주지 않습니다.
...
### 유사한 기술: 컨테이너(Container)

VM보다 훨씬 더 가볍게 동작하는 **컨테이너**도 널리 사용되고 있습니다. 컨테이너는 애플리케이션을 실행에 필요한 모든 요소(코드, 런타임, 시스템 도구, 라이브러리 등)와 함께 하나의 패키지로 묶어, 어떤 환경에서든 빠르고 안정적으로 실행될 수 있도록 하는 기술입니다.

컨테이너 기술의 가장 큰 특징은 **호스트 컴퓨터의 OS 커널을 공유**한다는 점입니다.

VM은 하드웨어 위에 하이퍼바이저를 설치하고, 그 위에 **독립된 게스트 OS를 설치**합니다. 이 구조는 완벽한 격리를 제공하지만, OS 라이선스 비용이 추가될 수 있으며 부팅 시간이 길고 시스템 자원을 많이 차지합니다.

반면 컨테이너는 호스트 OS 위에 컨테이너 엔진(예: Docker)을 설치하고, 각 컨테이너는 호스트 OS 커널을 공유합니다. OS를 새로 설치할 필요 없이 **애플리케이션과 그 실행에 필요한 종속성(Dependency)만 격리된 공간**에 담습니다.
...
- **비교적 낮은 보안성:** 모든 컨테이너가 호스트 OS 커널을 직접 공유하므로, 만약 커널에 보안 취약점이 발생한다면 해당 호스트에서 실행 중인 모든 컨테이너가 위험에 노출될 수 있습니다. 반면 VM은 하이퍼바이저를 통해 하드웨어 수준까지 격리되므로 보안성이 더 높습니다.
...
컨테이너와 관련된 대표적인 도구 두 가지로 Docker와 Kubernetes가 있습니다.

**Docker**는 컨테이너를 만들고, 실행하고, 관리하는 데 사용되는 **컨테이너화 플랫폼**입니다. 개발자가 `Dockerfile`이라는 파일에 실행 환경을 명시하면 이를 기반으로 Docker는 컨테이너 이미지(설계도)를 생성하고, 이 이미지를 실행하여 컨테이너를 만듭니다.

**Kubernetes**는 여러 서버(클러스터)에 걸쳐 수많은 컨테이너를 효과적으로 관리하고 조율(Orchestration)하기 위한 플랫폼입니다. 오케스트라의 지휘자처럼 Kubernetes는 컨테이너들을 자동으로 배포, 확장하고 트래픽을 분산하며 문제가 생긴 컨테이너를 자동으로 복구하는 등 **대규모 프로덕션 환경을 안정적으로 운영**하는 데 필수적인 역할을 합니다.
