...
* **핵심 아이디어:** 이미지를 한 번에 생성하는 것이 아니라, '완전한 노이즈'에서 시작해 **단계별로 노이즈를 제거(Denoising)**하여 이미지를 '조각'해냅니다.
    * **비유:** 빗소리(노이즈)만 가득한 녹음 파일에서, 빗소리를 섬세하게 단계적으로 제거했더니 원래는 들리지 않던 '사람의 목소리'가 드러나는 것과 같습니다.

### 어떻게 학습할까요? ― U-Net과 안정적인 손실 함수

Diffusion의 학습 과정은 크게 2단계로 나눌 수 있습니다.
...
* 이 과정을 1,000번 반복(Step 1,000)하면, 원본은 형체를 알아볼 수 없는 **'완전한 노이즈'**가 됩니다.

> **추후 업데이트 예정입니다.**
>
> 이 부분(Forward Diffusion)을 포함한 Diffusion 섹션 일부에 '수학적 보강'이 계획되어 있습니다.
{: .prompt-info }

#### 2. 역방향 확산(Reverse Diffusion): 노이즈를 예측해 복원하는 과정

* 이것이 **AI 모델(U-Net)이 학습하는 진짜 과정**입니다.
...
* **구조:** U-Net의 기본 구조는 AE처럼 '인코더-디코더'를 가집니다. (이미지의 문맥을 압축했다가 다시 펼침)

* **핵심:** ([1부](/posts/ai1-how-does-ai-understand-language/)에서 소개했던) ResNet처럼 **'스킵 커넥션(Skip Connection)'**을 가지고 있습니다.
...
* 완전히 무작위인 **'순수 노이즈'**에서 시작합니다. (Step 1,000)
...
* 이 과정을 남은 Step만큼 반복하여 노이즈가 단계적으로 걷히면, 최종적으로 선명한 이미지가 나타납니다.
...
이후 생성 단계에서 모델은 **두 가지 예측을 동시에** 수행합니다(CFG Scale).

* **$Pred_{cond}$**: "코딩하는 고양이" 프롬프트가 **있는** 상태의 노이즈 예측
* **$Pred_{uncond}$**: 프롬프트가 **없는** (무작위) 상태의 노이즈 예측

그리고 이 두 예측 사이의 '차이 벡터'($Pred_{cond} - Pred_{uncond}$)를 계산합니다. 이 벡터는 무작위(Random) 이미지에서 '고양이' 이미지로 향하는 **방향**을 의미합니다.

* **최종 예측:** `Final_Pred = Pred_uncond + scale * (Pred_cond - Pred_uncond)`

여기서 `scale`이 바로 사용자가 조절하는 **CFG Scale** 값(보통 7~15)입니다.

* **Scale 값이 높으면** 무작위 상태에서 '고양이' 방향으로 더 강하게 밀어붙여 **프롬프트에 충실한** 이미지를 만듭니다.
